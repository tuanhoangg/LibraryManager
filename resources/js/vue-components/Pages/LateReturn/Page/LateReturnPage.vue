                ...
                <div class="my-3 mx-3 row col-md-12">
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <label class="input-group-text" for="inputGroupSelect02"><i class="fas fa-filter"></i></label>
                        </div>
                        <select class="custom-select" v-model="status">
                            <option selected value="">Tất cả</option>
                            <option v-for="(status, key) in JSON.parse(listStatus)" :value="key">
                                {{status.label}}
                            </option>
                        </select>
                    </div>
            ...
            <table class="table text-nowrap table-bordered">
                ...
                <tbody>
                    ...
                    <tr v-for="(borrow, index) in lateReturn">
                        ...
                        <td class="text-wrap">{{ borrow.borrow.book.title }}</td>
                        <td class="text-wrap">{{ formatDate(borrow.borrow.borrow_date)}}</td>
                        <td class="text-wrap">{{ formatDate(borrow.borrow.due_date )}}</td>
...
export default {
    ...
    methods: {
        async loadItems() {
            ...
            const response = await axios.get(`late-returns/api/get-late-return-list?limit=${this.perPage}&offset=${(this.currentPage - 1) * this.perPage}&searchValue=${this.searchValue}
            &sortBy=${this.sortKey.key}&orderBy=${this.sortKey.type}&borrowStatus=${this.status}`);
